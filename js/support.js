...
function putInfoInModal(info) {
    info.messages.forEach((item) => {
        ...
        $('#modalDialogBody').before(
            `<div class="modal-message ${className}">
...
function setDialogsContent(data) {
    const answeredList = $('#questionsAnsweredList');
    const unansweredList = $('#questionsUnansweredList');
    const questionsPlace = $('#questionsPlace');

    if (data.content.length) {
        data.content.forEach((item) => {
            if (item.messages[item.messages.length - 1].type === 'question' && !item.closed) {
                unansweredList.append(`<li><a data-toggle="modal" data-target="#dialogModal" 
                 data-id="${item.id}" data-closed="${item.closed}">${item.title}</a></li>`);
            } else {
                answeredList.append(`<li><a data-toggle="modal" data-target="#dialogModal" 
                 data-id="${item.id}" data-closed="${item.closed}">${item.title}</a></li>`);
            }
        });
    } else {
        questionsPlace.html('').append('<span>You have not any questions now.</span>');
    }
}